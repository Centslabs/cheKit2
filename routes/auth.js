...
router.post("/user/reg", signUp) // signup 이라는 미들웨어를 signup 의 주소와 연결 시켜준다. 즉 front-end 클라이언트서버는 홈페이지주소/auth/signup 으로 회원가입 요청을 보낼 수 있게 된다.
router.post("/auth/login", signIn); // /auth/signin 엔드포인트로 들어오는 요청을 처리할 수 있도록 위에서 작성한 signIn 미들웨어를 연결시켜 준다.
